...
@endif
                <div class="intro-y flex items-center mt-8">
                    <h1 class="text-2xl font-medium leading-none">Edit Order</h1>
                </div>
                <form action="{{route('warehouse.order.update', $order->id )}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Order Number</p>
                        <input type="number" name="order_num" value="{{ $order->order_num}}" class="input w-full border mt-2">
                    </div>
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Order Date</p>
                        <input value="{{$order->order_date}}" name="order_date" class="datepicker input w-56 border block mx-auto" data-single-mode="true" style="width:100%;">
                    </div>
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Company</p>
                        <select data-search="true" class="tail-select w-full" name="company_id" style="width:100%;">
                            @foreach($company as $temp)
                                @if($order->company_id==$temp->id)
                    ...
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        ...
                        <select data-search="true" class="tail-select w-full"  name="order_user_id" style="width:100%;">
                            @foreach($order_user as $temp)
                                @if($order->order_user_id==$temp->id)
                    ...
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        ...
                        <select data-search="true" class="tail-select w-full"  name="supply_user_id" style="width:100%;">
                            @foreach($supply_user as $temp)
                                @if($order->supply_user_id==$temp->id)
                                <option value="{{$temp->id}}" selected>{{$temp->username}}</option>
                                @else
                                <option value="{{$temp->id}}">{{$temp->username}}</option>
                                @endif
                    ...
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Supply Date</p>
                        <input value="{{$order->supply_date}}" name="supply_date" class="datepicker input w-56 border block mx-auto" data-single-mode="true" style="width:100%;">
                    </div>
