...
@endif
                <div class="intro-y flex items-center mt-8">
                    <h1 class="text-2xl font-medium leading-none">Edit Item</h1>
                </div>
                <form action="{{route('warehouse.item.update', $item->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                    <div class="col-span-12 sm:col-span-4 xl:col-span-3 flex flex-col justify-end items-center">
                        <div class="upload mt-4 pr-md-4" style="width:200px;">
                            <input type="file" id="input-file-max-fs" name = "image" class="dropify" data-default-file="{{asset($item->image)}}" data-max-file-size="5M" />
                    ...
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>BarCode</p>
                        <input type="text" name="barcode" value="{{$item->barcode}}" class="input w-full border mt-2">
                    </div>
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Title</p>
                        <input type="text" name="title" value="{{ $item->title}}" class="input w-full border mt-2">
                    </div>
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        <p>Company</p>
                        <select data-search="true" class="tail-select w-full" name="company_id" style="width:100%;">
                            @foreach($company as $temp)
                                @if($item->company_id == $temp->id)
                    ...
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end ">
                        ...
                        <select data-search="true" class="tail-select w-full"  name="measurement_id" style="width:100%;">
                            @foreach($measurement as $temp)
                                @if($item->measurement_id == $temp->id)
                                <option value="{{$temp->id}}" selected>{{$temp->title}}</option>
                                @else
                                <option value="{{$temp->id}}">{{$temp->title}}</option>
                                @endif
