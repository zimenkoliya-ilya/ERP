...
@section('content')  
                <div class="intro-y flex items-center mt-8">
                    <h1 class="text-2xl text-theme-9 font-medium leading-none">Edit Bed</h1>
                </div>
                <form action="{{route('camp.bedlist.update',$ct_bed->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                        <label>Title</label>
                        <input type="text" name="title" value="{{$ct_bed->title}}" class="input w-full border mt-2" placeholder="title">
                    </div>
                    <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                        <label>Room</label>
                        <select data-search="true" class="tail-select w-full" name="room_id" style="width:100%;">
                            @foreach($ct_room as $temp)
                                @if($ct_bed->room_id == $temp->id)
