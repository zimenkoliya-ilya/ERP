...
<div class="layout-px-spacing">
    <div class="intro-y flex items-center mt-8">
        <h5 class="text-2xl font-medium leading-none">Model types</h5>
    </div>
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0 ">
        ...
        <div class="modal" id="new_category">
            <div class="modal__content" style="width:50%;">
                ...
                <form action="{{route('setting.model.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                        ...
                        <div class="col-span-12 sm:col-span-6"> 
                            <label>Make</label>
                            <select data-search="true" class="tail-select w-full mt-2"  name="make_id" style="width:100%;">
                                @foreach($make as $temp)
                                <option value="{{$temp->id}}">{{$temp->make}}</option>
                                @endforeach
                            </select> 
                        </div> 
                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex justify-center items-center">
                                        ...
                                        <a href="{{ route('setting.model.destroy', $temp->id)}}"  onclick="return confirm('Really');"  class="flex items-center text-theme-6 mr-3" href="javascript:;">
                                    ...
                                    <div class="modal" id="new_category{{$i}}">
                                        <div class="modal__content" style="width:50%;">
                                            ...
                                            <form action="{{route('setting.model.update', $temp->id)}}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                <div class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                                                    <div class="col-span-12 sm:col-span-6"> 
                                                        <label>Model</label> 
                                                        <input type="text" name="model" value="{{$temp->model}}" class="input w-full border mt-2">
                                                    </div> 
                                                    <div class="col-span-12 sm:col-span-6"> 
                                                        <label>Make</label>
                                                        <select data-search="true" class="tail-select w-full mt-2"  name="make_id" style="width:100%;">
                                                            @foreach($make as $tempp)
                                                                @if($temp->make_id == $tempp->id)
                                                                <option value="{{$tempp->id}}" selected>{{$tempp->make}}</option>
                                                                @else
                                                                <option value="{{$tempp->id}}" >{{$tempp->make}}</option>
                                                                @endif
                                                            @endforeach
                                                        </select> 
                                                    </div> 
