...
@endif
                <div class="intro-y flex items-center mt-8">
                    <h1 class="text-2xl font-medium leading-none">Update Position</h1>
                </div>
                <form action="{{route('setting.position.update',$position->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                        <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                            <label>Title</label>
                            <input type="text" name="title" value="{{ $position->title}}" class="input w-full border mt-2" placeholder="title">
                        </div>
                        <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                            <label>Department</label>
                            <select data-search="true" class="tail-select w-full"  name="department_id" style="width:100%;">
                                @foreach($department as $temp)
                                    @if($position->department_id == $temp->id)
                        ...
                        <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                            ...
                            <select data-search="true" class="tail-select w-full"  name="company_id" style="width:100%;">
                                @foreach($company as $temp)
                                    @if($position->company_id == $temp->id)
                                    <option value="{{$temp->id}}" selected>{{$temp->title}}</option>
                                    @else
                                    <option value="{{$temp->id}}">{{$temp->title}}</option>
                                    @endif
                        ...
                        <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                            ...
                            <select data-search="true" class="tail-select w-full"  name="available_position" style="width:100%;">
                                @foreach($avail_position as $temp)
                                    @if($position->available_position == $temp->id)
                                    <option value="{{$temp->id}}" selected>{{$temp->title}}</option>
                                    @else
                                    <option value="{{$temp->id}}">{{$temp->title}}</option>
                                    @endif
