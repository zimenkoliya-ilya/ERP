...
@endif
                <div class="intro-y flex items-center mt-8">
                    <h1 class="text-2xl font-medium leading-none">Update Owner Status Type</h1>
                </div>
                <form action="{{route('setting.owner_status.update',$owner_status->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="intro-y grid grid-cols-12 sm:gap-6 gap-y-6 box px-5 py-8 mt-5">
                        <div class="col-span-6 sm:col-span-6 xl:col-span-3 flex flex-col justify-end items-center">
                            <label>Type</label>
                            <input type="text" name="title" value="{{ $owner_status->title}}" class="input w-full border mt-2" placeholder="title">
                        </div>
