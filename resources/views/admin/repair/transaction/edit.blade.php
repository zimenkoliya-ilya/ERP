...
@endif
        ...
        <div class="intro-y flex items-center mt-8">
            <h4 class="leading-none">Update Replacement Transaction</h4>
        </div>
        <div class="intro-y box">
            <div class="p-5">
                <form action="{{route('repair.transaction.update', $trans->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="mt-2">
                                <select name="journal_id"  data-search="true" class="tail-select w-full" required="required">
                                    @foreach($journal as $temp)
                                        @if($trans->journal_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->date}}/{{$temp->customer->customer_name}}</option>
                                        @else
                            ...
                            <div class="mt-2">
                                <select name="repair_id"  data-search="true" class="tail-select w-full" required="required">
                                    @foreach($repair as $temp)
                                        @if($trans->repair_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->customer->customer_name}}/{{$temp->employee->first_name}}</option>
                                        @else
