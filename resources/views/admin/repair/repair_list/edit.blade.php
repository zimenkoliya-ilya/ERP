...
@endif
        ...
        <div class="intro-y box">
            <div class="p-5">
                <form action="{{route('repair.list.update', $repair->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-12 gap-4 gap-y-5" id="repair">
                            ...
                            <div class="mt-2">
                                <select data-search="true" name="customer_id" data-search="true" class="tail-select w-full" required="required">
                                    ...
                                    @foreach($customer as $temp)
                                        @if($repair->customer_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->customer->customer_name}}</option>
                                        @else
                            ...
                            <div class="mt-2">
                                <select name="employee_id"  data-search="true" class="tail-select w-full" required="required">
                                    ...
                                    @foreach($employee as $temp)
                                        @if($repair->employee_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->first_name}}/{{$temp->registration_num}}</option>
                                        @else
                        ...
                        <div class="intro-y col-span-3">
                            ...
                            <input style="border-color:lightgray !important;" name="start_datetime" type="datetime-local" value="{{$repair->start_datetime}}" id="DateInput" class="input w-full border mt-2 block" autocomplete="off" required="required">                        
                        </div>
                        <div class="intro-y col-span-3">
                            <span>Time Type</span>
                            <div class="mt-2">
                                <select name="time_type_id"  data-search="true" class="tail-select w-full" required="required">
                                    ...
                                    @foreach($time_type as $temp)
                                        @if($repair->time_type_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->title}}</option>
                                        @else
                                        <option value="{{$temp->id}}">{{$temp->title}}</option>
                                        @endif
                        ...
                        <div class="intro-y col-span-3">
                            ...
                            <input style="border-color:lightgray !important;" type="text" name="repair_reason" value="{{ $repair->repair_reason}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>Description</span>
                            <input style="border-color:lightgray !important;" type="text" name="repair_description" value="{{ $repair->repair_description}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>Shift</span>
                            <input style="border-color:lightgray !important;" type="text" name="shift_id" value="{{ $repair->shift_id}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>In Moto Hour</span>
                            <input style="border-color:lightgray !important;" type="text" name="in_moto_hour" value="{{ $repair->in_moto_hour}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>Out Moto Hour</span>
                            <input style="border-color:lightgray !important;" type="text" name="out_moto_hour" value="{{ $repair->out_moto_hour}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>In Speedometer</span>
                            <input style="border-color:lightgray !important;" type="text" name="in_speedometer" value="{{ $repair->in_speedometer}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>On Speedometer</span>
                            <input style="border-color:lightgray !important;" type="text" name="on_speedometer" value="{{ $repair->on_speedometer}}" class="input w-full border mt-2">
                        </div>
                        <div class="intro-y col-span-3">
                            <span>Category</span>
                            <div class="mt-2">
                            ...
                            <div class="mt-2">
                                <select name="repair_type_id"  data-search="true" class="tail-select w-full" required="required">
                                    ...
                                    @foreach($type as $temp)
                                        @if($repair->repair_type_id == $temp->id)
                                        <option value="{{$temp->id}}" selected>{{$temp->title}}</option>
                                        @else
                                        <option value="{{$temp->id}}">{{$temp->title}}</option>
                                        @endif
